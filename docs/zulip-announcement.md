...
- `#general` → topic *"Sound Wolfram bridge for Lean 4"*, or
...
**A sound Wolfram bridge for Lean 4 — the CAS discovers, the kernel verifies**

Hi all — I'd like to share a Lean 4 ↔ Wolfram (Mathematica) bridge I've been building.
It began as a Lean 4 + mathlib4 port of **Robert Y. Lewis and Minchao Wu's** Lean 3
*MM-Lean* interface ([robertylewis/mathematica](https://github.com/robertylewis/mathematica)) —
the original Lean ↔ Mathematica bridge — and I've extended it since. The design
principle is that the tactics are **sound**: Wolfram proposes a certificate or a
...
- **`mathematica_ring`** — Wolfram finds a `PolynomialReduce` certificate and
  `ring` / `linear_combination` verifies it (a Wolfram-powered `polyrith`).
- **`mathematica_rw`** — a fixed-point subterm-rewriting loop: simplify a subterm in
  Wolfram, validate the step with `ring` / `field_simp` / `norm_num` / `simp`,
...
- **`LeanCheck`** (on the Wolfram side) — the reverse direction: verify a Wolfram
...
-- sound: a binomial sum whose certificate is found by Wolfram
example (n : ℕ) : ∑ k ∈ Finset.range (n + 1), (n.choose k)^2 = (2 * n).choose n := by
  mathematica_telescope
```

The flagship is a certified, end-to-end proof of `∑ C(n,k)² = C(2n,n)`: Wolfram
